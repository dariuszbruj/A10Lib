...
A10GPIO::A10GPIO()
{
	...
	// Set default pointer to nullptr
	this->_gpio_address = nullptr;

	// get platform page size ( on CB it should be 4kb)
	...
	this->_gpio_size = ((unsigned int)( A10MEMORY::PORTC_IO_SIZE / this->_page_size)+1)*this->_page_size;
...
uint32_t A10GPIO::init()
{
	...
	start  = A10MEMORY::PORTC_IO_BASE &  this->_page_mask;
	offset = A10MEMORY::PORTC_IO_BASE & ~this->_page_mask;
...
uint8_t  A10GPIO::isInitialized()
{
	return this->_isInitialized;
}

void 	 A10GPIO::toggle(uint16_t port, uint16_t number)
{
...
void 	 A10GPIO::toggle(A10::PIN portpin)
{
	if (!this->_isInitialized)
		return;
...
void 	 A10GPIO::toggle_f(volatile uint32_t *DATREG, uint32_t PIN)
{
	// Toggle port.
	*DATREG ^= PIN;
}

volatile uint32_t *	A10GPIO::datareg(uint16_t port)
{
	// Get DAT register for selected port.
	return this->_gpio_address + port*9 + 4;
}

volatile uint32_t *	A10GPIO::datareg(A10::PIN portpin)
{
	// Get DAT register for selected port.
	return this->datareg(this->portnum(portpin));
}

uint16_t		A10GPIO::portnum(A10::PIN portpin)
{
	return ((uint16_t) portpin >> 8);
}

uint16_t		A10GPIO::pinnum(A10::PIN portpin)
{
	return ((uint16_t) portpin & 0xFF);
}

uint8_t  A10GPIO::read(uint16_t port, uint16_t number)
{
	if (!this->_isInitialized)
		return -1;
...
uint8_t  A10GPIO::read(A10::PIN portpin)
{
	if (!this->_isInitialized)
		return -1;
...
void 	 A10GPIO::write(uint16_t port, uint16_t number, uint8_t value)
{
	if (!this->_isInitialized)
		return;
...
void 	 A10GPIO::write(A10::PIN portpin, uint8_t value)
{
	if (!this->_isInitialized)
		return;
...
void	 A10GPIO::clear(uint16_t port, uint16_t number)
{
	if (!this->_isInitialized)
		return;
...
void	 A10GPIO::clear(A10::PIN portpin)
{
	if (!this->_isInitialized)
		return;
...
void 	 A10GPIO::select(uint16_t port, uint16_t number, uint8_t mode)
{
	if (!this->_isInitialized)
		return;

	volatile uintptr_t* CFG = this->_gpio_address + port*9 + (number/8); 		// port*0x24 + offset (0x0, 0x4, 0x8, 0xC)
...
void 	 A10GPIO::select(A10::PIN portpin, A10::PINMODE  mode)
{
	if (!this->_isInitialized)
		return;
